...
GRAPH_INPUT_PATH = '../Data/Graph/'
MODEL_OUTPUT_DIR = '../models/MAF-TAV-mBART/'
RESULT_OUTPUT_DIR = '../results/MAF-TAV-mBART/'
EMO_PATH = '../Data/Video_emotion/'
TONE_PATH = '../Data/Audio_tone/'
SENT_PATH = '../Data/Text/'
...
set_random_seed(random.randint(0, 9999))
...
class MultimodalMBartEncoder(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # torch.nn.TransformerEncoderLayer(d_model=VISUAL_DIM, nhead=8,num_encoder_layers=4, dim_feedforward=VISUAL_DIM)
        ...
        self.acoustic_transformer = TransformerEncoder(d_model=ACOUSTIC_DIM,
                                                       ...
                                                       n_heads=2,
                                                       d_ff=ACOUSTIC_DIM)
        #  torch.nn.TransformerEncoderLayer(d_model=VISUAL_DIM, nhead=8,num_layers=4, dim_feedforward=VISUAL_DIM)
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if idx in self.fusion_at_layer:
                ...
                text_adj = graph
                text_gcn_feat = F.relu(self.gc1(text_gcn, graph))
...
class GraphConvolution(nn.Module):
    def __init__(self, in_features, out_features, bias=False):
        ...
        self.weight = nn.Parameter(torch.eye(1024))  # torch.FloatTensor(in_features, out_features)
        if bias:
...
def prepare_dataset(text_path: str,
    ...
    adj = pkl.load(adj)
    emo_data = pd.read_pickle(emo_path)  # video
    tone_data = pd.read_pickle(tone_path)  # audio
    sent_data = pd.read_pickle(sent_path)  # text
    if unfolded_dialogue:
        dialogue = []
        adj_all = []
        for i in range(1, len(data) + 1):
            emo_i = emo_data[i]  # len 1
            tone_i = tone_data[i]  # len 2
            sent_i = sent_data[i]  # len 1
            ...
            example_dialogue = example_dialogue + "[EMOTION]"
            for ii in range(len(sent_i)):
                example_dialogue = example_dialogue + ' ' + sent_i[ii] + ' ' + emo_i[ii] + ' ' + tone_i[ii]
        ...
        emo_i = emo_data[cnt]  # len 1
        tone_i = tone_data[cnt]  # len 2
        sent_i = sent_data[cnt]  # len 1
        adj_all = []
        target = []
        context = []
        for i in range(1, len(data) + 1):
            ...
            for ii in range(len(sent_i)):
                context = context + ' ' + sent_i[ii] + ' ' + emo_i[ii] + ' ' + tone_i[ii]
...
def preprocess_dataset(dataset,
    ...
    model_inputs['attention_mask'] = torch.tensor([a for a in model_inputs['attention_mask']], dtype=torch.long,
                                                  device=DEVICE)
...
def get_val_scores(model,
    ...
    if "Validation" in desc:
        ...
        if result['bleu_4']>max_test['bleu_4']:
            file_name = RESULT_OUTPUT_DIR + "best_" + str(epoch + 1) + "_val_results.csv"
            val_df.to_csv(file_name, index=False)
        ...
        if result['bleu_4']>max_test['bleu_4']:
            file_name = RESULT_OUTPUT_DIR + "best_" + str(epoch + 1) + "_test_results.csv"
            test_df.to_csv(file_name, index=False)
...
def train(model,
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        path = MODEL_OUTPUT_DIR + "MAF_TAV_mBART_epoch_" + TARGET_COLUMN + "_epoch_" + str(
...
if __name__ == "__main__":
    ...
    MODEL = MultimodalMBartForConditionalGeneration.from_pretrained(
        '')
    print("Model loaded...\n")
    MODEL.to(DEVICE)
    ...
    TOKENIZER = MBart50TokenizerFast.from_pretrained('',
                                                     src_lang="en_XX",
                                                     tgt_lang="en_XX")  # 'facebook/mbart-large-50-many-to-many-mmt'
